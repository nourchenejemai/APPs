...
function Geologie() {
  const [geologies, setGeologies] = useState([]);
  ...
  const fetchGeologies = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      
      const url = searchTerm 
        ? `http://localhost:8080/api/geologie/${searchTerm}`
        : `http://localhost:8080/api/geologie?page=${page}&limit=${limit}`;
      ...
      if (searchTerm) {
        ...
        if (data.message === "Geologie not found") {
          setGeologies([]);
        } else {
          setGeologies([data]);
        }
        ...
        setGeologies(data.geologies || data);
        setTotalPages(data.totalPages || 1);
      }
    } catch (error) {
      console.error("Error fetching geologies:", error);
  ...
  const handleDelete = async (id, geologieDesc) => {
    const confirmDelete = window.confirm(`Are you sure you want to delete "${geologieDesc || 'this geology'}" (ID: ${id})?`);
    if (!confirmDelete) return;

    try {
      const response = await fetch(`http://localhost:8080/api/geologie/${id}`, { 
        method: 'DELETE' 
      });
  ...
  const handleFileUpload = async (event) => {
    ...
    const confirmUpload = window.confirm(`Are you sure you want to upload "${file.name}"? This will add new geology data.`);
    ...
    try {
      ...
      const response = await fetch('http://localhost:8080/api/geologie/upload', {
        method: 'POST',
        body: formData,
      });
      ...
      if (response.ok) {
        ...
        fetchGeologies(); // Refresh the data
      } else {
        const errorData = await response.json();
  ...
  const handleDownloadCSV = async () => {
    try {
      ...
      const response = await fetch('http://localhost:8080/api/geologie?limit=10000');
      ...
      const geologiesData = data.geologies || data;
      
      if (!geologiesData || geologiesData.length === 0) {
        alert('No data available to download');
        return;
      }

      // Create CSV content
      const headers = ['ID', 'Superficie', 'Age', 'Lithologie', 'Code', 'Descript'];
      const csvHeaders = headers.join(',');
      
      const csvRows = geologiesData.map(geologie => [
        geologie.id || '',
        geologie.superficie || '',
        geologie.age || '',
        geologie.lithologie || '',
      ...
      link.setAttribute('download', `geologie_data_${new Date().toISOString().split('T')[0]}.csv`);
      ...
      alert(`Downloaded ${geologiesData.length} records successfully!`);
  ...
  const columns = [
    ...
    { key: 'superficie', label: 'Superficie', width: '100px' },
    { key: 'age', label: 'Age', width: '100px' },
    { key: 'lithologie', label: 'Lithologie', width: '150px' },
    { key: 'code', label: 'Code', width: '100px' },
    { key: 'descript', label: 'Description', width: '200px' },
  ];
  ...
  return (
    <div className="flex-1 p-6 space-y-6 overflow-auto">
      ...
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-bold text-gray-800">Management Geologie Bizerte</h2>
        <div className="flex gap-2">
          ...
          <button
            ...
            disabled={downloadLoading || geologies.length === 0}
            className={`flex items-center bg-purple-600 hover:bg-purple-700 text-white font-semibold py-2 px-4 rounded-lg shadow transition duration-200 ${
              (downloadLoading || geologies.length === 0) ? 'opacity-50 cursor-not-allowed' : ''
            }`}
          >
      ...
      {!loading && !error && geologies.length === 0 && (
        <div className="text-center py-8 text-gray-500">
          No geologie data found
        </div>
      )}

      {/* Table */}
      {!loading && geologies.length > 0 && (
        <>
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full">
                ...
                <tbody className="divide-y divide-gray-200">
                  {geologies.map((geologie) => (
                    <tr key={geologie.id} className="hover:bg-gray-50">
                      {columns.map((column) => (
                      ...
                      <td className="p-3 flex space-x-2">
                        <button
                          onClick={() => navigate('/ModifyGeologie', { state: { geolog: geologie } })}
                        ...
                        <button
                          onClick={() => handleDelete(geologie.id, geologie.descript)}
