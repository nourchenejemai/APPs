...
function Barrages() {
  ...
  const fetchBarrages = useCallback(async () => {
    try {
      ...
      const url = searchTerm 
        ? `http://localhost:8080/api/barrages/${searchTerm}`
        : `http://localhost:8080/api/barrages?page=${page}&limit=${limit}`;
      ...
      if (searchTerm) {
        ...
        if (data.message === "Barrages not found") {
          setBarrages([]);
        } else {
          setBarrages([data]);
        }
        ...
        setBarrages(data.barrages || data);
  ...
  const handleDelete = async (gid, barrageName) => {
    const confirmDelete = window.confirm(`Are you sure you want to delete "${barrageName}" (ID: ${gid})?`);
  ...
  const handleFileUpload = async (event) => {
    ...
    const confirmUpload = window.confirm(`Are you sure you want to upload "${file.name}"? This will add new barrage data.`);
    ...
    try {
      ...
      const response = await fetch('http://localhost:8080/api/barrages/upload', {
        method: 'POST',
        body: formData,
      });
      ...
      if (response.ok) {
        ...
        fetchBarrages(); // Refresh the data
      } else {
        const errorData = await response.json();
  ...
  const handleDownloadCSV = async () => {
    try {
      ...
      const response = await fetch('http://localhost:8080/api/barrages?limit=10000');
      ...
      const barragesData = data.barrages || data;
      
      if (!barragesData || barragesData.length === 0) {
        alert('No data available to download');
        return;
      }

      // Create CSV content
      const headers = ['GID', 'Name', 'Name_Ar', 'Name_Fr', 'Type_Bge', 'Gouver', 'Code', 'Disc'];
      const csvHeaders = headers.join(',');
      
      const csvRows = barragesData.map(barrage => [
        barrage.gid || '',
        barrage.name || '',
        barrage.name_ar || '',
        barrage.name_fr || '',
      ...
      link.setAttribute('download', `barrages_data_${new Date().toISOString().split('T')[0]}.csv`);
      ...
      alert(`Downloaded ${barragesData.length} records successfully!`);
  ...
  const columns = [
    ...
    { key: 'name_ar', label: 'Nom Ar', width: '150px' },
    { key: 'name_fr', label: 'Nom Fr', width: '150px' },
    { key: 'type_bge', label: 'Type', width: '120px' },
    { key: 'gouver', label: 'Gouvernorat', width: '130px' },
    { key: 'code', label: 'Code', width: '100px' },
    { key: 'disc', label: 'Description', width: '200px' },
  ];
  ...
  return (
    <div className="flex-1 p-6 space-y-6 overflow-auto">
      ...
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-bold text-gray-800">Management Barrages</h2>
        <div className="flex gap-2">
          ...
          <button
            ...
            disabled={downloadLoading || barrages.length === 0}
            className={`flex items-center bg-purple-600 hover:bg-purple-700 text-white font-semibold py-2 px-4 rounded-lg shadow transition duration-200 ${
              (downloadLoading || barrages.length === 0) ? 'opacity-50 cursor-not-allowed' : ''
            }`}
          >
      ...
      {!loading && !error && barrages.length === 0 && (
              ...
              <table className="min-w-full">
                ...
                <tbody className="divide-y divide-gray-200">
                  {barrages.map((barrage) => (
                    <tr key={barrage.gid} className="hover:bg-gray-50">
                      {columns.map((column) => (
                      ...
                      <td className="p-3 flex space-x-2">
                        <button
                          onClick={() => navigate('/ModifyBarrage', { state: { barrage } })}
                        ...
                        <button
                          onClick={() => handleDelete(barrage.gid, barrage.name)}
