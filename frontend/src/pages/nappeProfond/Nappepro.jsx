...
function Nappepro() {
  const [nappesp, setNappesp] = useState([]);
  ...
  const fetchNappesp = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      
      const url = searchTerm 
        ? `http://localhost:8080/api/nappesp/${searchTerm}`
        : `http://localhost:8080/api/nappesp?page=${page}&limit=${limit}`;
      ...
      if (searchTerm) {
        ...
        if (data.message === "Nappe profonde not found") {
          setNappesp([]);
        } else {
          setNappesp([data]);
        }
        ...
        setNappesp(data.nappesp || data);
        setTotalPages(data.totalPages || 1);
      }
    } catch (error) {
      console.error("Error fetching nappes profondes:", error);
  ...
  const handleDelete = async (id, nappeName) => {
    ...
    try {
      const response = await fetch(`http://localhost:8080/api/nappesp/${id}`, { 
        method: 'DELETE' 
      });
  ...
  const handleFileUpload = async (event) => {
    ...
    const confirmUpload = window.confirm(`Are you sure you want to upload "${file.name}"? This will add new deep water table data.`);
    ...
    try {
      ...
      const response = await fetch('http://localhost:8080/api/nappesp/upload', {
        method: 'POST',
        body: formData,
      });
  ...
  const handleDownloadCSV = async () => {
    try {
      ...
      const response = await fetch('http://localhost:8080/api/nappesp?limit=10000');
      ...
      const nappespData = data.nappesp || data;
      
      if (!nappespData || nappespData.length === 0) {
        alert('No data available to download');
        return;
      }

      // Create CSV content
      const headers = ['ID', 'Surface', 'Perimetre', 'NPRIDE', 'NPRNom', 'NPRCod', 'NPRRes', 'NPRExp', 'NPRQMI', 'NPRQMA'];
      const csvHeaders = headers.join(',');
      
      const csvRows = nappespData.map(nappe => [
        nappe.id || '',
        nappe.surface || '',
        nappe.perimetre || '',
        nappe.npride || '',
      ...
      link.setAttribute('download', `deep_water_table_data_${new Date().toISOString().split('T')[0]}.csv`);
      ...
      alert(`Downloaded ${nappespData.length} records successfully!`);
  ...
  const columns = [
    ...
    { key: 'npride', label: 'NPR IDE', width: '100px' },
    { key: 'nprnom', label: 'NPR Nom', width: '150px' },
    { key: 'nprcod', label: 'NPR Code', width: '100px' },
    { key: 'nprres', label: 'NPR Res', width: '100px' },
    { key: 'nprexp', label: 'NPR Exp', width: '100px' },
    { key: 'nprqmi', label: 'NPR QMI', width: '100px' },
    { key: 'nprqma', label: 'NPR QMA', width: '100px' },
  ];
  ...
  return (
    <div className="flex-1 p-6 space-y-6 overflow-auto">
      ...
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-bold text-gray-800">Management Nappe Profonde</h2>
        <div className="flex gap-2">
          ...
          <button
            ...
            disabled={downloadLoading || nappesp.length === 0}
            className={`flex items-center bg-purple-600 hover:bg-purple-700 text-white font-semibold py-2 px-4 rounded-lg shadow transition duration-200 ${
              (downloadLoading || nappesp.length === 0) ? 'opacity-50 cursor-not-allowed' : ''
            }`}
          >
      ...
      {!loading && !error && nappesp.length === 0 && (
                  ...
                  {nappesp.map((nappe) => (
                    <tr key={nappe.id} className="hover:bg-gray-50">
                      ...
                      <td className="p-3 flex space-x-2">
                        <button
                          onClick={() => navigate('/ModifyNappep', { state: { nappe } })}
                        ...
                        <button
                          onClick={() => handleDelete(nappe.id, nappe.nprnom)}
