...
function Vertisol() {
  const [vertisols, setVertisols] = useState([]);
  ...
  const fetchVertisols = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      
      const url = searchTerm 
        ? `http://localhost:8080/api/vertisols/${searchTerm}`
        : `http://localhost:8080/api/vertisols?page=${page}&limit=${limit}`;
      ...
      if (searchTerm) {
        ...
        if (data.message === "Vertisol not found") {
          setVertisols([]);
        } else {
          setVertisols([data]);
        }
        ...
        setVertisols(data.vertisols || data);
        setTotalPages(data.totalPages || 1);
      }
    } catch (error) {
      console.error("Error fetching vertisols:", error);
  ...
  const handleDelete = async (id, vertisolColor) => {
    const confirmDelete = window.confirm(`Are you sure you want to delete vertisol with color "${vertisolColor || 'N/A'}" (ID: ${id})?`);
    if (!confirmDelete) return;

    try {
      const response = await fetch(`http://localhost:8080/api/vertisols/${id}`, { 
        method: 'DELETE' 
      });
  ...
  const handleFileUpload = async (event) => {
    ...
    const confirmUpload = window.confirm(`Are you sure you want to upload "${file.name}"? This will add new vertisol data.`);
    ...
    try {
      ...
      const response = await fetch('http://localhost:8080/api/vertisols/upload', {
        method: 'POST',
        body: formData,
      });
      ...
      if (response.ok) {
        ...
        fetchVertisols(); // Refresh the data
      } else {
        const errorData = await response.json();
  ...
  const handleDownloadCSV = async () => {
    try {
      ...
      const response = await fetch('http://localhost:8080/api/vertisols?limit=10000');
      ...
      const vertisolsData = data.vertisols || data;
      
      if (!vertisolsData || vertisolsData.length === 0) {
        alert('No data available to download');
        return;
      }

      // Create CSV content
      const headers = ['ID', 'Couleur', 'TypeCoul'];
      const csvHeaders = headers.join(',');
      
      const csvRows = vertisolsData.map(vertisol => [
        vertisol.id || '',
        vertisol.couleur || '',
        vertisol.typecoul || ''
      ].join(','));
      ...
      link.setAttribute('download', `vertisol_data_${new Date().toISOString().split('T')[0]}.csv`);
      ...
      alert(`Downloaded ${vertisolsData.length} records successfully!`);
  ...
  const columns = [
    ...
    { key: 'couleur', label: 'Couleur', width: '120px' },
    { key: 'typecoul', label: 'Type Couleur', width: '150px' },
  ];
  ...
  return (
    <div className="flex-1 p-6 space-y-6 overflow-auto">
      ...
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-bold text-gray-800">Management Vertisol</h2>
        <div className="flex gap-2">
          ...
          <button
            ...
            disabled={downloadLoading || vertisols.length === 0}
            className={`flex items-center bg-purple-600 hover:bg-purple-700 text-white font-semibold py-2 px-4 rounded-lg shadow transition duration-200 ${
              (downloadLoading || vertisols.length === 0) ? 'opacity-50 cursor-not-allowed' : ''
            }`}
          >
      ...
      {!loading && !error && vertisols.length === 0 && (
        <div className="text-center py-8 text-gray-500">
          No vertisol data found
        </div>
      )}

      {/* Table */}
      {!loading && vertisols.length > 0 && (
        <>
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full">
                ...
                <tbody className="divide-y divide-gray-200">
                  {vertisols.map((vertisol) => (
                    <tr key={vertisol.id} className="hover:bg-gray-50">
                      {columns.map((column) => (
                      ...
                      <td className="p-3 flex space-x-2">
                        <button
                          onClick={() => navigate('/ModifyVertisol', { state: { vert: vertisol } })}
                        ...
                        <button
                          onClick={() => handleDelete(vertisol.id, vertisol.couleur)}
