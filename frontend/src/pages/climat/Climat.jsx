...
function Climat() {
  ...
  const fetchClimats = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      
      const url = searchTerm 
        ? `http://localhost:8080/api/climat/${searchTerm}`
        : `http://localhost:8080/api/climat?page=${page}&limit=${limit}`;
      ...
      if (searchTerm) {
        ...
        if (data.message === "Climat not found") {
          setClimats([]);
        } else {
          setClimats([data]);
        }
  ...
  const handleDelete = async (id, climatName) => {
    const confirmDelete = window.confirm(`Are you sure you want to delete "${climatName || 'this climat'}" (ID: ${id})?`);
    if (!confirmDelete) return;

    try {
      const response = await fetch(`http://localhost:8080/api/climat/${id}`, { 
        method: 'DELETE' 
      });
  ...
  const handleFileUpload = async (event) => {
    ...
    const confirmUpload = window.confirm(`Are you sure you want to upload "${file.name}"? This will add new climat data.`);
    ...
    try {
      ...
      const response = await fetch('http://localhost:8080/api/climat/upload', {
        method: 'POST',
        body: formData,
      });
  ...
  const handleDownloadCSV = async () => {
    try {
      ...
      const response = await fetch('http://localhost:8080/api/climat?limit=10000');
      ...
      const climatsData = data.climats || data;
      
      if (!climatsData || climatsData.length === 0) {
        alert('No data available to download');
        return;
      }

      // Create CSV content
      const headers = ['ID', 'Surface', 'Perimetre', 'CLM IDE', 'CLM Nom', 'CLM Cla', 'CLM Mox'];
      const csvHeaders = headers.join(',');
      
      const csvRows = climatsData.map(climat => [
        climat.id || '',
        climat.surface || '',
        climat.perimetre || '',
        climat.clmide || '',
      ...
      link.setAttribute('download', `climat_data_${new Date().toISOString().split('T')[0]}.csv`);
      ...
      alert(`Downloaded ${climatsData.length} records successfully!`);
  ...
  const columns = [
    ...
    { key: 'clmide', label: 'CLM IDE', width: '100px' },
    { key: 'clmnom', label: 'CLM Nom', width: '150px' },
    { key: 'clmcla', label: 'CLM Cla', width: '100px' },
    { key: 'clmmox', label: 'CLM Mox', width: '100px' },
  ];
  ...
  return (
    <div className="flex-1 p-6 space-y-6 overflow-auto">
      ...
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-bold text-gray-800">Management Climat Bizerte</h2>
        <div className="flex gap-2">
          ...
          <button
            ...
            disabled={downloadLoading || climats.length === 0}
            className={`flex items-center bg-purple-600 hover:bg-purple-700 text-white font-semibold py-2 px-4 rounded-lg shadow transition duration-200 ${
              (downloadLoading || climats.length === 0) ? 'opacity-50 cursor-not-allowed' : ''
            }`}
          >
      ...
      {!loading && !error && climats.length === 0 && (
        <div className="text-center py-8 text-gray-500">
          No climat data found
        </div>
      )}

      {/* Table */}
      {!loading && climats.length > 0 && (
        <>
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full">
                ...
                <tbody className="divide-y divide-gray-200">
                  {climats.map((climat) => (
                    <tr key={climat.id} className="hover:bg-gray-50">
                      {columns.map((column) => (
                      ...
                      <td className="p-3 flex space-x-2">
                        <button
                          onClick={() => navigate('/ModifyClimat', { state: { climat } })}
                        ...
                        <button
                          onClick={() => handleDelete(climat.id, climat.clmnom)}
